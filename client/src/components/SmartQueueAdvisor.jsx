...
const SmartQueueAdvisor = ({ centreId, selectedDate, onSelectBestSlot }) => {
  ...
  if (loading) {
    return (
      <div className="bg-gradient-to-br from-white to-green-50/50 rounded-3xl p-6 border-2 border-green-200 shadow-md animate-pulse space-y-4 text-left">
        <div className="flex justify-between items-center">
          <div className="h-4 w-40 bg-green-100 rounded-md"></div>
  ...
  if (!advisorData) {
    return (
      <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm text-left">
        <div className="flex items-center space-x-2 text-gray-500">
          <Sparkles className="h-4 w-4 text-green-600" />
          <span className="text-xs font-bold">{t('smartQueueAdvisorTitle')}</span>
        </div>
        <p className="text-3xs text-gray-400 mt-2">{t('estimateUnavailable')}</p>
      </div>
    );
  }
  ...
  const renderCongestionBadge = (level) => {
    if (level === 'HIGH') {
      return (
        <span className="inline-flex items-center gap-1 font-extrabold text-red-800 bg-red-100 border border-red-200 px-2.5 py-0.5 rounded-full text-4xs">
    ...
    if (level === 'MODERATE') {
      return (
        <span className="inline-flex items-center gap-1 font-extrabold text-amber-900 bg-amber-100 border border-amber-200 px-2.5 py-0.5 rounded-full text-4xs">
          🟡 {t('moderateCrowd') || t('congestionModerate')}
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 font-extrabold text-green-800 bg-green-100 border border-green-200 px-2.5 py-0.5 rounded-full text-4xs">
  ...
  return (
    <div className="bg-gradient-to-br from-white via-[#fafdf9] to-emerald-50/40 rounded-3xl p-5 sm:p-6 border-2 border-green-200 shadow-md space-y-5 text-left transition hover:shadow-lg">
      
      {/* Top Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-green-100/70 pb-3">
        <div className="flex items-center space-x-2.5">
          <span className="p-2 bg-green-600 text-white rounded-xl shadow-xs">
            <Sparkles className="h-4 w-4" />
          </span>
          <div>
            <h3 className="font-extrabold text-xs sm:text-sm text-gray-900 flex items-center gap-1.5">
              <span>{t('smartQueueAdvisorTitle')}</span>
              <span className="text-3xs font-semibold px-2 py-0.5 bg-green-100 text-green-800 rounded-full border border-green-200">
                AI Mandi Engine
              </span>
            </h3>
            <p className="text-4xs text-gray-500">{centreName || t('smartQueueAdvisorSubtitle')}</p>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          <span className="text-4xs font-bold text-gray-400">{t('currentCongestionLevel')}:</span>
          {renderCongestionBadge(congestionLevel)}
        </div>
      </div>

      {/* Featured "⭐ Best Time to Visit" Recommendation Block */}
      <div className="bg-gradient-to-r from-emerald-600 to-green-700 rounded-2xl p-4 sm:p-5 text-white shadow-md relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left">
        ...
        <div className="bg-white/80 p-3 rounded-2xl border border-green-100 shadow-2xs">
          ...
          <span className="text-base sm:text-lg font-black text-gray-900 block">
            {currentQueue} <span className="text-3xs font-medium text-gray-500">{t('farmersWaiting')}</span>
          </span>
        </div>
        ...
        <div className="bg-white/80 p-3 rounded-2xl border border-green-100 shadow-2xs">
          ...
          <span className="text-base sm:text-lg font-black text-blue-900 block">
            ~{expectedWaitTime} <span className="text-3xs font-bold text-blue-700">{t('minutes')}</span>
          </span>
        </div>
