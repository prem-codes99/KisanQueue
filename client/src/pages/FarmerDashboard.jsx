    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-left space-y-6">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-200/80 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1 flex flex-wrap items-center gap-x-2">
            <span>{t('village')}: <b className="text-gray-700">{user?.profile?.village}</b></span>
            <span>•</span>
            <span>{t('farmerId')}: <b className="text-gray-700 font-mono">{user?.profile?.farmerId}</b></span>
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2.5">
          {!isInstalled && (
            <button
              onClick={installApp}
              className="inline-flex items-center px-4 py-2 border border-gray-300 text-xs font-semibold rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition shadow-2xs cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {activeBooking ? (
            <div className="bg-white rounded-2xl border border-gray-200/90 shadow-xs overflow-hidden">
              {/* Card Header */}
              <div className="bg-gray-50/90 px-5 sm:px-6 py-3.5 border-b border-gray-200/80 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-sm text-gray-800">{t('todayStatus')}</h3>
                  <p className="text-3xs text-gray-500 font-mono">ID: {activeBooking.bookingId}</p>
                </div>
                <span className={`text-3xs font-bold px-3 py-1 rounded-full uppercase tracking-wider ${
                  activeBooking.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                  activeBooking.status === 'IN_QUEUE' || activeBooking.status === 'PROCESSING' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
              ...
              <div className="p-5 sm:p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
                    <span className="text-4xs uppercase font-bold text-emerald-700 tracking-wider block">{t('tokenNumber')}</span>
                    <span className="text-lg font-black text-emerald-800 block mt-0.5">{activeBooking.tokenNumber}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-200/70">
                    <span className="text-4xs uppercase font-bold text-gray-500 tracking-wider block">{t('crop')}</span>
                    <span className="text-sm font-bold text-gray-800 block mt-0.5">{t(`crop${activeBooking.cropType.replace(/[^a-zA-Z]/g, '')}`, activeBooking.cropType)}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-200/70">
                    <span className="text-4xs uppercase font-bold text-gray-500 tracking-wider block">{t('quantity')}</span>
                    <span className="text-sm font-bold text-gray-800 block mt-0.5">{activeBooking.approxQuantity} {t('quintals')}</span>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-200/70">
                    <span className="text-4xs uppercase font-bold text-gray-500 tracking-wider block">{t('slotTime')}</span>
                    <span className="text-sm font-bold text-gray-800 block mt-0.5">{activeBooking.slotId?.startTime} - {activeBooking.slotId?.endTime}</span>
                  </div>
                </div>

                {/* Progress Journey Stepper */}
                <div className="border-t border-gray-100 pt-4">
                  <h4 className="text-3xs font-bold text-gray-500 uppercase tracking-wider mb-3">{t('journeyTrackerTitle')}</h4>
                        ...
                        return (
                          <div key={idx} className="flex flex-col items-center relative z-10 px-1">
                            <div 
                              ...
                              } ${isActive ? 'ring-3 ring-emerald-100 font-extrabold' : ''}`}
                            >
                              {isCompleted ? '✓' : step.stepNum}
                            </div>
                            <span className={`text-4xs mt-1 font-semibold whitespace-nowrap ${
                              isActive ? 'text-emerald-700 font-bold' : isCompleted ? 'text-emerald-600' : 'text-gray-400'
                            }`}>
                              {step.label}
                            </span>
                          </div>
                ...
                {/* Location Bar */}
                <div className="bg-gray-50/80 p-3 rounded-xl border border-gray-200/70 flex items-center space-x-2.5 text-left">
                  <MapPin className="h-4 w-4 text-emerald-600 shrink-0" />
                  <div className="min-w-0">
                    <span className="text-xs font-bold text-gray-800 truncate block">{activeBooking.centreId?.name}</span>
                    <span className="text-3xs text-gray-500 truncate block">{activeBooking.centreId?.location}</span>
                  </div>
                </div>
                ...
                {liveQueue && (
                  <div className="border-t border-gray-100 pt-4 space-y-4">
                    <h4 className="text-3xs font-bold text-gray-500 uppercase tracking-wider">{t('queueTimelineTitle')}</h4>
                    
                    <div className="grid grid-cols-3 gap-2.5 text-center">
                      <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-200/70">
                        <span className="text-4xs uppercase font-bold text-gray-500 block truncate">{t('currentServing')}</span>
                        <span className="text-sm font-bold text-gray-800 block mt-0.5 truncate">{liveQueue.currentServingToken}</span>
                      </div>
                      <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-200/70">
                        <span className="text-4xs uppercase font-bold text-gray-500 block truncate">{t('queuePosition')}</span>
                        <span className="text-sm font-bold text-emerald-700 block mt-0.5 truncate">#{liveQueue.position}</span>
                      </div>
                      <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-200/70">
                        <span className="text-4xs uppercase font-bold text-gray-500 block truncate">{t('estWaitTime')}</span>
                        <span className="text-sm font-bold text-gray-800 block mt-0.5 truncate">{liveQueue.estimatedWaitTime} {t('minutes')}</span>
                      </div>
                    </div>

                    {/* Clearance Chain Tracker */}
                    <div className="p-3 bg-gray-50 rounded-xl border border-gray-200/70 text-center">
                      <span className="text-3xs font-bold text-gray-600 uppercase tracking-wider mb-2 block">{t('clearanceChainTitle')}</span>
                      <div className="flex flex-wrap items-center justify-center gap-1.5 text-xs font-medium">
                        ...
                        <span className="bg-white border border-gray-300 px-2 py-0.5 rounded text-3xs">{t('waitingAhead')}: {liveQueue.position - 1}</span>
                        <span className="text-gray-400 text-xs">➔</span>
                        <span className="bg-emerald-100 text-emerald-800 border border-emerald-300 px-2 py-0.5 rounded font-bold text-3xs">{user?.profile?.name?.split(' ')?.[0] || t('farmer')}: {liveQueue.tokenNumber}</span>
                      </div>
                    </div>

                    {/* Smart Recommendations */}
                    {getIdealReachTime() && (
                      <div className="bg-emerald-50/70 p-3 rounded-xl border border-emerald-200 flex items-start space-x-2 text-left">
                        <span className="text-base shrink-0">💡</span>
                        <p className="text-3xs text-emerald-950 leading-relaxed">
                          {t('arrivalRecommendationText')} <b className="font-bold text-emerald-900">{getIdealReachTime()}</b> {t('arrivalRecommendationBuffer')}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {activeBooking.status === 'COMPLETED' && (
                  <div className="border-t border-gray-100 pt-4 space-y-3 text-left">
                    <div className="flex justify-between items-center">
                      <h4 className="text-3xs font-bold text-gray-500 uppercase tracking-wider">{t('billingReceiptTitle')}</h4>
                      <span className="text-3xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full">
                        ✓ {t('COMPLETED')}
                      </span>
                    </div>

                    <div className="bg-gray-50 border border-gray-200/80 rounded-xl p-3.5 grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        <span className="text-4xs uppercase font-bold text-gray-500 block">{t('totalPayoutBilling')}</span>
                      ...
                      <div>
                        <span className="text-4xs uppercase font-bold text-gray-500 block">{t('payoutStatus')}</span>
                        <span className={`inline-block text-4xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full mt-1 ${
                          activePayment?.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' :
                          activePayment?.status === 'PROCESSING' ? 'bg-blue-100 text-blue-800' :
                      ...
                      <div>
                        <span className="text-4xs uppercase font-bold text-gray-500 block">{t('transactionRef')}</span>
                        <span className="text-3xs font-mono text-gray-600 block mt-1">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 rounded-lg text-xs font-semibold transition cursor-pointer shadow-2xs"
                      >
                ...
                {activeBooking.status === 'BOOKED' && (
                  <div className="flex justify-end border-t border-gray-100 pt-3">
                    <button
                      ...
                      disabled={cancelling}
                      className="px-3.5 py-1.5 border border-red-200 rounded-lg text-xs font-semibold text-red-600 bg-white hover:bg-red-50 transition disabled:opacity-50 cursor-pointer"
                    >
                      {t('cancelBookingBtn')}
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-2xl p-8 border border-gray-200/80 shadow-xs text-center space-y-3">
              ...
              <h3 className="text-base font-bold text-gray-900">{t('noBooking')}</h3>
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-xs">
            <h3 className="font-bold text-sm text-gray-800 flex items-center gap-1.5 mb-3">
              <FileText className="h-4 w-4 text-emerald-600" /> {t('historyTitle')}
            </h3>

            {bookings.length === 0 ? (
              <p className="text-gray-400 text-xs py-4 text-center">{t('historyEmpty')}</p>
            ) : (
                  ...
                  <tbody className="divide-y divide-gray-100 text-xs">
                    {bookings.map((b) => (
                      <tr key={b._id} className="hover:bg-gray-50/70">
                        <td className="py-2.5 px-2 text-gray-600">{b.date}</td>
                        <td className="py-2.5 px-2 font-medium text-gray-800">{b.centreId?.name}</td>
                        <td className="py-2.5 px-2 text-gray-700">{t(`crop${b.cropType.replace(/[^a-zA-Z]/g, '')}`, b.cropType)}</td>
                        <td className="py-2.5 px-2 text-gray-600">{b.approxQuantity} {t('quintals')}</td>
                        ...
                        <td className="py-2.5 px-2 text-right">
                          {b.status === 'COMPLETED' ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-700 rounded-md text-3xs font-semibold transition cursor-pointer"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-xs">
            <h3 className="font-bold text-sm text-gray-800 flex items-center gap-1.5 mb-3">
              <HelpCircle className="h-4 w-4 text-emerald-600" /> {t('faqTitle')}
            </h3>

            <div className="space-y-2.5">
              {[1, 2, 3].map((num) => (
                <div key={num} className="border-b border-gray-100 pb-2.5 last:border-0">
                  <button
                    onClick={() => toggleFaq(num)}
                    className="w-full flex justify-between items-center text-left text-xs font-semibold text-gray-700 hover:text-emerald-700 focus:outline-none cursor-pointer"
                  >
                    <span>{t(`faq${num}Q`)}</span>
                    <span className="text-gray-400 text-3xs ml-2">{openFaq === num ? '▲' : '▼'}</span>
                  </button>
                  {openFaq === num && (
                    <p className="text-3xs text-gray-600 mt-1.5 leading-relaxed bg-gray-50 p-2.5 rounded-lg border border-gray-100">
                      {t(`faq${num}A`)}
                    </p>
                  )}
                </div>
          ...
          <div className="bg-emerald-950 text-white rounded-2xl p-5 space-y-3 text-left shadow-xs">
