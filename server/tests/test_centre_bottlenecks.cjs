...
async function runTests() {
  ...
  const testPhone = '91234' + randNum;
  const testName = 'Nashik Agro Grain Hub ' + randNum;
  console.log(`2. Public Centre Registration for: ${testName} (${testPhone})...`);
  const regPayload = {
    name: testName,
  ...
  const createdCentreId = regRes.data.data.centreId;
  console.log(`✓ Centre registered successfully! ID: ${createdCentreId}, Initial Status: ${regRes.data.data.status}`);
  ...
  const operatorToken = approvedLoginRes.data.token;
  console.log(`✓ Operator successfully logged in! Role: ${approvedLoginRes.data.user.role}, Centre: ${approvedLoginRes.data.user.profile?.centreId?.name}\n`);
